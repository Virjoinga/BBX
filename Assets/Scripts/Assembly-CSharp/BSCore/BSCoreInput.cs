...
namespace BSCore
{
	public class BSCoreInput
	{
		...
		{
			PlayerSaveData saveData = Player.GetSaveData(userAssignableMapsOnly: false);
			...
			if (list.Count < fields.Count)
			{
				Debug.LogError($"{fields.Count - list.Count} options are not mapped in saved data. Resetting the mappings.");
				ResetToDefault();
			}
		}
