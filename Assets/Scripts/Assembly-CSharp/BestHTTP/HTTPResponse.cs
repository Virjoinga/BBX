...
namespace BestHTTP
{
	public class HTTPResponse : IDisposable
	{
		...
		{
			string text = ReadTo(stream, 10).Split(';')[0];
		...
		private byte[] Decompress(byte[] data, int offset, int count)
		{
			...
			{
				decompressorGZipStream = new GZipStream(decompressorInputStream, CompressionMode.Decompress, BestHTTP.Decompression.Zlib.CompressionLevel.Default, leaveOpen: true);
